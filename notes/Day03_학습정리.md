...
| 방식 | 원리 | 강점 | 약점 |
|---|---|---|---|
| Sparse Embedding | 단어 출현 빈도 기반 (TF-IDF, BM25) | 고유명사·코드·정확한 키워드 매칭에 강함 | 동의어·표현 차이에 약함 ("아이들" ≠ "어린이") |
| Dense Embedding | 신경망이 문장 의미를 수백~수천 차원 벡터로 압축 | 의미가 비슷하면 벡터도 가까워짐 — 동의어·바꿔 말하기에 강함 | 정확한 키워드/고유명사 매칭은 상대적으로 약함 |

- 실습에 쓰는 `Qwen3-Embedding-8B`는 **Dense** 방식
- 예: "아이들이 공원에서 놀고 있다" vs "어린이들이 파크에서 뛰어놀고 있어" → Sparse는 겹치는 단어가 없어 "남남"으로 판단, Dense는 거의 같은 문장으로 인식
- 그래서 "검색 방식 비교" 표의 하이브리드 검색(BM25+벡터)이 이 둘을 합쳐서 서로의 약점을 보완하는 것
